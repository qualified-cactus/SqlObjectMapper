...
class KotlinClassMapping<T : Any>(private val clazz: Class<T>) : ClassMapping<T> {
    ...
    init {
        ...
        fun findProperty(name: String): KProperty1<T, *> {
            return propertyNames[name]
                ?: throw SqlObjectMapperException("Can't find property ${name} in ${kClazz}")
        }

        for (param in primaryConstructor.parameters) {
            val column = param.findAnnotation<Column>()
            val oneToMany = param.findAnnotation<OneToMany>()
            ...
            if (column != null) {
                val columnName = if (column.name == "")
                    NameConverter.convertName(param.name!!)
                else column.name
                ...
                if (colNameToPropertyMap.containsKey(columnName)) {
                    throw SqlObjectMapperException("Duplicate column/parameter name \"${columnName}\" found in ${kClazz}")
                }

                if (column.isId) {
                    if (idColumnName != null) {
                        throw SqlObjectMapperException("Duplicate ID column \"${columnName}\" in ${kClazz}")
                    }
                ...
                if (nestedClassMapping.idColumnName != null) {
                    if (idColumnName != null) {
                        throw SqlObjectMapperException("Duplicate ID column \"${nestedClassMapping.idColumnName}\" in ${nestedClazz}")
                    }
                    idColumnName = nestedClassMapping.idColumnName
                }

                for ((colName, nestedPropertyInfo) in nestedClassMapping.colNameToPropertyMap) {
                    if (colNameToPropertyMap.containsKey(colName)) {
                        throw SqlObjectMapperException("Duplicate column/parameter name \"${colName}\" found in ${nestedClazz} nested inside ${clazz}")
                    }
                ...
                val columnName = NameConverter.convertName(param.name!!)
                val property = findProperty(param.name!!)
                colNameToPropertyMap[columnName] = PropertyInfo(
                    {o -> property.getter(o as T)},
                    ValueConverter()
    ...
    override val idColumn: String
        get() = idColumnName ?: throw SqlObjectMapperException("${kClazz} doesn't have an ID column")
    ...
    private data class CollectionParam(
        ...
        val elementConverter: ValueConverter
    )
